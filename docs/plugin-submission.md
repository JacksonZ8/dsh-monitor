...
> 复制下面 `---` 之后的正文到 issue，标题填 `[收录] dsh-monitor`。
...
- 对 `workflow` 通过一个**显式进度协议**（脚本 `log('dsh-monitor:total=N')` 声明批次总数，每个 `agent()` 完成即推进 `done`）计算**进度条 + `done/total` + 动态 ETA**；
...
- **是否已在 DSH 上实测 / verified on DSH?**：是（本地 `dsh plugin --profile test add ./dsh-monitor` + `dsh --profile test --port 0`，验证了 bundle 组装、browser bundle serve、`GET /dsh-monitor/snapshot` 返回 JSON）。

## 备注 / Notes

- **license**：MIT
- **维护状态**：新项目，单作者
- **双半插件**：host 半（`dist/index.js`，`exports["."]`，`inject: [webServer, timer]`）+ 浏览器半（`dist/client.js`，`window.__ModuleLoader__.load({id:'dsh-monitor', factory})`），遵循 `dsh-plugin-tutorial` 面板规范。
- **已知坑**：`shell.overlay` 为 `list` 槽，本插件用独立 `id: dsh-monitor`，不与内置条目冲突。
- **进度协议约定**：workflow 脚本在派发批次前 `log('dsh-monitor:total=N')`；未声明 total 时回退 `meta.phases` 数量，仍无则只显示耗时。
