...
把 `dsh-progress-monitor`（npm 包名，仓库名 `dsh-monitor`）在 DSH web profile 里真实装载并验证面板 + 数据通道。
...
# 或从本地 checkout 安装（npm 包名仍为 dsh-progress-monitor）：
cd /path/to/dsh-monitor && cd ..
...
预期启动日志出现：
- loader 装载 `dsh-progress-monitor` 行（来自 `cordis.patch.yml` 的 `insert`）
- host 半 `apply` 执行（`webServer` 路由注册，无输出也正常）
- 浏览器半 factory 物化（`shell.overlay` 注册 `id: dsh-progress-monitor`）
...
```bash
curl -s http://127.0.0.1:3080/dsh-progress-monitor/snapshot
# 预期返回 JSON：{"now":..., "items":[...]}
```
...
2. 点 pill 展开面板，空载显示「暂无长任务」。
3. 拖 pill 或面板 header，位置应跟手，刷新后经纬度记忆生效（双击复位）。
...
# workflow（进度条 + done/total + 剩余≈）
# 让 agent 用 workflow 工具跑带进度协议的脚本：
#   log('progress:total=N') 后在循环里 await agent(...)
```

预期：
- 后台 job 行显示「任务」徽章 + 命令 + 耗时，状态 ●/✓/✕；
- workflow 行显示「工作流」徽章 + 进度条 + `done/total` + `剩余≈`。

## 5. 若面板不显示 / 数据为空

按以下顺序排查：

1. **host 路由有没有**：`curl .../dsh-progress-monitor/snapshot` 是否 200/JSON。
2. **browser 半 factory 是否物化**：浏览器 console 有无
   `window.__ModuleLoader__.load` 相关报错；确认 `id: 'dsh-progress-monitor'` 与包名一致。
...
- `ctx.styles.insert` 在浏览器半 factory 的 `apply(ctx)` 里是否可用：本包已
  ...
  skill 的 Builtin 列表）。
- `fetch('/dsh-progress-monitor/snapshot')` 的相对路径在 web 部署下是否命中 host 路由
